...
3. On boot, oauth.json is deleted and the process exits.
4. The space auto-restarts into Phase 1 setup.
5. Edit the repo's ``reset-setup.ini`` back to ``PURGE_OAUTH=0`` so
   the next rebuild does not re-trigger the reset.
...
def _flag_roots() -> tuple[str, ...]:
    ...
    return _FLAG_ROOTS_STATIC + (os.getcwd(),)


# ── helpers ──────────────────────────────────────────────────────


def _find(path: str, roots: tuple[str, ...]) -> str | None:
    for root in roots:
